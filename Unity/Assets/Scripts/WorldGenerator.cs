...
public class WorldGenerator : MonoBehaviour
{
    ...
    [SerializeField] Tile treeLeavesM;
    [SerializeField] Tile treeLeavesML;
    [SerializeField] Tile treeLeavesMR;
    [SerializeField] Tile treeLeavesB;
    [SerializeField] Tile treeLeavesBL;
    [SerializeField] Tile treeLeavesBR;
    [SerializeField] Tile treeLeavesT;
    [SerializeField] Tile treeLeavesTL;
    [SerializeField] Tile treeLeavesTR;
    Camera mainCamera;
    Player player;
    ...
    void DisplayTree(int treeHeight, bool spawnLeftStump, bool spawnRightStump, Vector3Int startingPosition)
    {
        for (int i = 0; i < treeHeight; i++)
        {
            if (i == 0)
            {
                backgroundTilemap.SetTile(startingPosition, treeStumpM);

                if (spawnLeftStump)
                {
                    backgroundTilemap.SetTile(startingPosition + new Vector3Int(-1, 0, 0), treeStumpBL);
                }

                if (spawnRightStump)
                {
                    backgroundTilemap.SetTile(startingPosition + new Vector3Int(1, 0, 0), treeStumpBR);
                }
            }
            else if (treeHeight - i > 3)
            {
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(0, i, 0), treeTrunk);
            }
            else if (treeHeight - i == 3)
            {
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(0, i, 0), treeLeavesB);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(1, i, 0), treeLeavesBR);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(-1, i, 0), treeLeavesBL);
            }
            else if (treeHeight - i == 2)
            {
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(0, i, 0), treeLeavesM);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(1, i, 0), treeLeavesMR);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(-1, i, 0), treeLeavesML);
            }
            else
            {
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(0, i, 0), treeLeavesT);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(1, i, 0), treeLeavesTR);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(-1, i, 0), treeLeavesTL);
            }
    ...
    void HideTree(int treeHeight, bool spawnLeftStump, bool spawnRightStump, Vector3Int startingPosition)
    {
        for (int i = 0; i < treeHeight; i++)
        {
            if (i == 0)
            {
                backgroundTilemap.SetTile(startingPosition, null);

                if (spawnLeftStump)
                {
                    backgroundTilemap.SetTile(startingPosition + new Vector3Int(-1, 0, 0), null);
                }
            ...
            {
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(0, i, 0), null);
            }
            else if (treeHeight - i == 3)
            {
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(0, i, 0), null);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(1, i, 0), null);
                backgroundTilemap.SetTile(startingPosition + new Vector3Int(-1, i, 0), null);
            }
            else if (treeHeight - i == 2)
            {
        ...
        for (int x = 0; x < terrainWidth; x++)
        {
            ...
            if (Random.Range(0f, normalnoise) <= .02f)
            {
                if (Random.Range(0, 2) == 0)
                {
                    ...
                    backgroundTilemap.SetTile(new Vector3Int(x, highestTiles[x] + 1, 0), redFlowerTile);
                }
                else
                {
                    backgroundTilemap.SetTile(new Vector3Int(x, highestTiles[x] + 1, 0), yellowFlowerTile);
                }
            }
        }
    }
    ...
    void SpawnPlayer()
    {
        ...
        player.backgroundTilemap = backgroundTilemap;
    ...
    void LoadChunks()
    {
        if (!fillInWorld)
        {
            ...
            {
                for (int y = bottomLeftChunkLoadCoordinate.y; y <= topRightLoadPosition.y; y++)
                {
